...
class DB:
    def __init__(self):
        ...
        for path in [PATH_PACIENTES, PATH_ATENDIMENTOS, PATH_USUARIOS]:
            if not os.path.exists(path):
                with open(path, "w", encoding="utf-8") as f:
                    json.dump([], f)
        
        # Criar usuário admin padrão se não existir
        usuarios = self.get(PATH_USUARIOS)
        if not usuarios:
            admin = {
                "id": 1,
                "nome": "Administrador",
            ...
            self.save(PATH_USUARIOS, usuarios)
...
class UsuarioModel:
    ...
    def login(self, email, senha):
        ...
        usuarios = self.db.get(PATH_USUARIOS)
    ...
    def registrar(self, nome, email, senha, confirmar_senha):
        ...
        self.db.save(PATH_USUARIOS, usuarios)
...
class AtendimentoModel:
    ...
    def registrar(self, paciente_id, descricao, prioridade):
        ...
        if len(descricao.strip()) < 10:
            return False, "A descrição deve ter no mínimo 10 caracteres"
    ...
    def deletar(self, atendimento_id):
        ...
        return True, "Atendimento excluído"
...
class PacienteModel:
    ...
    def add(self, nome, nasc, tel, email, doc):
        ...
        if email and any(p.get("email") == email for p in data):
            return False, "Email já cadastrado"
    ...
    def deletar(self, paciente_id):
        # Verificar se paciente tem atendimentos
        atendimentos = self.db.get(PATH_ATENDIMENTOS)
        if any(a["paciente_id"] == paciente_id for a in atendimentos):
            return False, "Não é possível excluir paciente com atendimentos registrados"
...
def make_entry(master, placeholder="", width=260, show=None):
    return ctk.CTkEntry(
        master,
...
class LoginWindow(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        btn_registrar = ctk.CTkButton(
            ...
            text_color=CORES["primary"],
            hover_color=CORES["primary_light"],
            font=FONT_BODY_BOLD,
            height=40,
            width=300,
        )
    ...
    def fazer_login(self):
        ...
        ok, msg = self.parent.usuario_model.login(email, senha)
        
        if ok:
            self.alert_label.configure(text=f"✓ {msg}", text_color=CORES["success"])
            self.after(1500, self.login_sucesso)
        else:
            self.alert_label.configure(text=f"✗ {msg}", text_color=CORES["danger"])
    ...
    def abrir_registro(self):
        RegisterWindow(self.parent, self)
...
class RegisterWindow(ctk.CTkToplevel):
    def __init__(self, parent, login_window):
        ...
        self.title("Registro - MedSync Pro")
        ...
        self.nome_entry.pack(pady=(0, 15))
        ...
        self.confirmar_entry.pack(pady=(0, 20))
        
        # Dicas de senha
        tips_frame = ctk.CTkFrame(scroll, fg_color=CORES["info_light"], corner_radius=8)
    ...
    def fazer_registro(self):
        ...
        ok, msg = self.parent.usuario_model.registrar(nome, email, senha, confirmar)
        
        if ok:
            self.alert_label.configure(text=f"✓ {msg}", text_color=CORES["success"])
            self.after(1500, self.voltar_login)
        else:
            self.alert_label.configure(text=f"✗ {msg}", text_color=CORES["danger"])
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.usuario_model = UsuarioModel(self.db)
        ...
        self.main_interface_visible = False
    ...
    def abrir_login(self):
        ...
        if self.usuario_model.esta_logado():
            self.show_main_interface()
    
    def show_main_interface(self):
        if self.main_interface_visible:
            return
        
        self.main_interface_visible = True
        self.clear()
    ...
    def build_sidebar(self):
        ...
        usuario = self.usuario_model.get_usuario_logado()
        if usuario:
            user_frame = ctk.CTkFrame(self.sidebar, fg_color=CORES["sidebar_hover"], corner_radius=10)
            user_frame.pack(fill="x", padx=15, pady=(10, 15))
        ...
        logout_btn.pack(fill="x", padx=12, pady=(0, 10))
    ...
    def show_pacientes(self):
        ...
        cols = ["ID", "Nome", "Documento", "Telefone", "Cadastro", "Ações"]
    ...
    def show_atendimentos(self):
        ...
        area = self._content_area()
        self.header(area, "Atendimentos", "Histórico de atendimentos realizados")
    ...
    def show_form_atendimento(self):
        ...
        ctk.CTkLabel(
            form_inner, text="Descrição do Atendimento *", font=FONT_SMALL_BOLD, text_color=CORES["text"]
        ).pack(anchor="w", pady=(0, 5))
        ...
        def salvar():
            ...
            try:
                ...
                alert(form_alert, "Selecione um paciente válido", "erro")
                return
    ...
    def modal_atendimento(self, pid):
        ...
        ctk.CTkLabel(
            form_frame, text="Descrição *", font=FONT_SMALL_BOLD, text_color=CORES["text"]
        ).pack(anchor="w", pady=(0, 5))
        txt = ctk.CTkTextbox(
            form_frame,
            height=140,
        ...
        def salvar():
            ...
            if len(descricao) < 10:
                alert(modal_alert, "A descrição deve ter no mínimo 10 caracteres", "erro")
                return
            prioridade = cb.get()
            ok, m = self.atendimento_model.registrar(pid, descricao, prioridade)
            if ok:
